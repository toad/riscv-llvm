...
if spike pk test-main-bogus-vptr.cc.clang.riscv 2>&1 | grep "Segmentation fault"; then echo "Clang should prevent misaligned load"; exit 2; fi
...
if ! $TOP/lowrisc-chip/riscv-tools/run_test_linux_spike.sh $PWD/test-main-iostreams.cc.gcc-linux.riscv; then echo "Static initialisers broken with gcc-linux"; exit 6; fi
...
if spike pk test-main-read-only-vptr.cc.clang.riscv 2>&1 | grep "ERROR: Called invalid funcion"; then echo "Clang vptr overwriting test should prevent misaligned load"; exit 9; fi
