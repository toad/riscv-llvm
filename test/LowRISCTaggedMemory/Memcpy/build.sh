...
for x in object; do
	...
	clang -O0 -target riscv -mcpu=LowRISC -mriscv=LowRISC -I -I $RISCV/riscv64-unknown-elf/include/ -S ${x}.c -emit-llvm -o ${x}.ll || exit 2
...
for main in main*.c; do
	...
	do
		...
		if test "$build" = "gcc"; then
			...
			riscv64-unknown-elf-gcc -fpermissive -O0 -I $RISCV/riscv64-unknown-elf/include/ -S $main -o ${main}.s || exit 1
		else
			echo Building $main with clang
			if ! clang -O0 -target riscv -mcpu=LowRISC -mriscv=LowRISC -I $RISCV/riscv64-unknown-elf/include/ -S $main -emit-llvm -o ${main}.ll; then echo Failed to build $main with $build; break; fi
